...
tf_y=tf.placeholder(tf.float32,[None,11])
...
#13*13
conv3=tf.layers.conv2d(lrn2,128,8,2,padding='valid',activation=tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1,dtype=tf.float32))

conv4=tf.layers.conv2d(conv3,11,11,activation=tf.nn.relu,kernel_initializer=tf.truncated_normal_initializer(stddev=0.1,dtype=tf.float32))

output=tf.reshape(conv4,[-1,11])
...
def save(sess,i):
    ...
    saver.save(sess,r"F:\models\室内物体检测\model.ckpt")
